...
-- BƯỚC 5: Kiểm tra lại dữ liệu đã khớp nhau chưa
SELECT * FROM Contract;
GO


-- 3. Tạo lại bảng Contract chuẩn chỉnh
USE CarRentalDB;
CREATE TABLE Contract (
    ContractID INT IDENTITY(123,1) PRIMARY KEY, -- Tự động bắt đầu từ 123
    CustomerID INT NOT NULL,
    StartDate DATETIME2 NOT NULL,               -- Ngày bắt đầu
    EndDate DATETIME2 NOT NULL,                 -- Ngày kết thúc
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    UpdatedAt DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    FOREIGN KEY (CustomerID) REFERENCES Customer(CustomerID)
);
-- 5. Kiểm tra kết quả hiển thị dữ liệu
CREATE TABLE Notification (
    NotificationID INT IDENTITY(1,1) PRIMARY KEY,
    ContractID INT NOT NULL,
    Title NVARCHAR(255) DEFAULT N'Nhắc trả xe',
    Content NVARCHAR(MAX),
    SentAt DATETIME2 DEFAULT SYSDATETIME(),
    Status NVARCHAR(50) DEFAULT N'Sent',
...
('Pham',    'Quoc Huy',   'huy.pham@email.com',     '0934567890', '1992-07-12', 'VN-B2-0045678', '2028-07-12', 'VN'),
('Vo',      'Thanh Mai',  'mai.vo@email.com',       '0945678901', '1993-09-25', 'VN-B2-0056789', '2029-09-25', 'VN'),
('Hoang',   'Gia Bao',    'bao.hoang@email.com',    '0956789012', '1988-01-30', 'VN-B2-0067890', '2027-01-30', 'VN'),
('Do',      'Thu Trang',  'trang.do@email.com',     '0967890123', '1996-06-18', 'VN-B2-0078901', '2030-06-18', 'VN'),
('Bui',     'Anh Khoa',   'khoa.bui@email.com',     '0978901234', '1991-12-05', 'VN-B2-0089012', '2028-12-05', 'VN'),
('Dang',    'Ngoc Linh',  'linh.dang@email.com',    '0989012345', '1994-04-22', 'VN-B2-0090123', '2029-04-22', 'VN'),
('Phan',    'Tien Dat',   'dat.phan@email.com',     '0990123456', '1987-10-10', 'VN-B2-0101234', '2027-10-10', 'VN');
GO

INSERT INTO MaintenanceRecord (VehicleID, MaintenanceType, Description, MileageAtService, ServiceDate, NextServiceDate, Cost, ServiceProvider, CreatedAt)
VALUES
(1, N'Bảo dưỡng', N'Thay nhớt và lọc gió', 15000, '2026-05-01', DATEADD(day, 7, GETDATE()), 500000.00, N'Garage ABC', GETDATE()),
(2, N'Bảo dưỡng', N'Kiểm tra và bảo dưỡng phanh', 28000, '2026-05-10', DATEADD(day, 9, GETDATE()), 350000.00, N'Trung tâm dịch vụ XYZ', GETDATE()),
(3, N'Sửa chữa', N'Thay 4 lốp xe mới', 32000, '2026-05-15', DATEADD(day, 11, GETDATE()), 2500000.00, N'Garage Lốp Việt', GETDATE()),
(4, N'Bảo dưỡng', N'Bảo dưỡng hệ thống điều hòa', 8000, '2026-05-20', DATEADD(day, 13, GETDATE()), 700000.00, N'Điện lạnh ô tô', GETDATE()),
(5, N'Sửa chữa', N'Thay bình ắc quy', 41000, '2026-05-25', DATEADD(day, 15, GETDATE()), 1200000.00, N'Garage ABC', GETDATE());
GO

-- 1. Thêm 2 cột ngày vào bảng Contract
ALTER TABLE Contract ADD StartDate DATETIME2;
ALTER TABLE Contract ADD EndDate DATETIME2;
GO
UPDATE Contract SET StartDate = '2026-05-20', EndDate = '2026-05-27' WHERE ContractID = 123; -- Hết hạn sau 1 ngày
UPDATE Contract SET StartDate = '2026-05-15', EndDate = '2026-05-29' WHERE ContractID = 124; -- Hết hạn sau 3 ngày
UPDATE Contract SET StartDate = '2026-05-10', EndDate = '2026-05-26' WHERE ContractID = 125; -- Hết hạn HÔM NAY
UPDATE Contract SET StartDate = '2026-05-01', EndDate = '2026-05-28' WHERE ContractID = 126; -- Đã quá hạn 2 ngày
UPDATE Contract SET StartDate = '2026-05-20', EndDate = '2026-06-01' WHERE ContractID = 127; --
GO
